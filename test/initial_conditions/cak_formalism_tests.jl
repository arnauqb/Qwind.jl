...
@testset "Test Nozzle functions" begin
    ...
    model.wind = Wind(model.wind.wind_hull, dgrid, model.wind.velocity_grid, model.wind.vacuum_density, model.wind.update_grid_flag)
    r = 100
    fuv = 0.85
    rad = model.rad
    bh = model.bh
    ...
    @testset "Test gamma0" begin
        ...
        gamma0 = Qwind.gamma0(model.rad, parameters, r)
    ...
    @testset "Test f" begin
        ...
        for r in range(6.1, 1000.0, length = 50)
            @test Qwind.f(rad, parameters, 1e-2, r = r, alpha = 0.6) ≈ cc * fuv rtol = 5e-1
        end
    end

    @testset "Test g" begin
        # test grids are restored
        Qwind.g(rad, parameters, 1, r = 100)
        ...
        @test Qwind.g(rad, parameters, z, r = r) ≈ -(disk_grav + low_rad / fuv) / Qwind.get_B0(100)
