...
function plot_streamlines(streamlines, fig = nothing, ax = nothing; alpha = 1.0, xl=0, xh=1000, yl=0, yh=1000)
    color_scheme = colorschemes[:RdPu_9]
    if ax === nothing
        fig, ax = plt.subplots()
    end
    for (i, line) in enumerate(streamlines)
        color = get(color_scheme, i / length(streamlines))
        color = [color.r, color.g, color.b]
        #println(convert(Float64, color))
        ax.plot(line.r, line.z, "o-", color = color, alpha = alpha, markersize=0.2)
    end
    ax.set_xlabel("r [Rg]")
    ax.set_ylabel("z [Rg]")
    ax.set_xlim(xl, xh)
