...
export ConstantFUV, QsosedRadiation, from_quadtree, getfuv, compute_disk_temperature, getmdot, getridx
...
function QsosedRadiation(bh::BlackHole, nr::Int, fx::Float64)
    ...
    rmin = bh.isco
    rmax = 1400
    disk_grid, uv_fractions = sed.compute_uv_fractions(inner_radius=rmin, outer_radius=rmax, return_all=false, log_spaced=true)
    mdot_grid = bh.mdot .* ones(length(disk_grid))
    xray_luminosity = fx * compute_bolometric_luminosity(bh)
    return QsosedRadiation(disk_grid, uv_fractions, mdot_grid, xray_luminosity, sed, bh.efficiency, bh.spin, bh.isco, bh.Rg)
end
...
compute_disk_temperature(radiation::QsosedRadiation, r) = radiation.sed.disk_nt_temperature4(r)^(1.0 / 4.0)